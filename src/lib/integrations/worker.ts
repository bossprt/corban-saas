...
export const localProvidersAllowed=(env:LocalProviderEnv)=>env.NODE_ENV!=='production'&&env.CORBAN_ALLOW_LOCAL_PROVIDERS==='1'
...
export async function runDispatchCycle(deps:WorkerDeps):Promise<CycleSummary>{
 ...
 let items:DispatchItem[]=await deps.repo.listDispatchable(deps.onlyRunId?50:limit,now())
...
// Authorization of the dispatch trigger (route / CLI): shared secret compared in constant time. Unset secret = trigger disabled.
