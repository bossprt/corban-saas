...
export const ATTENTION_RULE_KEYS = ['overdue_cases', 'stale_leads', 'draft_proposals', 'pendencies_due', 'paid_without_receipt', 'receipt_divergence_open', 'deferred_missing', 'chargebacks', 'payout_approvals_pending', 'portal_submissions_pending', 'payouts_approved_unpaid'] as const
...
export async function loadAttention({ supabase, membership }: Ctx, opts: { withEvents?: boolean } = {}) {
  ...
  const [overdue, stale, drafts, pendencies, finance, payout, portal] = await Promise.all([
    ...
    supabase.from('proposal_submissions').select('proposal_id,created_at', exact).eq('status', 'pending').order('created_at').limit(200),
  ])
  ...
  const fin = (kind: string): Signal | null => {
  ...
  const data: AttentionData = {
    ...
    portalSubmissions: portal.error || portal.count === null ? null
      : { count: portal.count, ids: (portal.data ?? []).slice(0, 5).map(r => r.proposal_id), oldest: portal.data?.[0]?.created_at ?? null },
