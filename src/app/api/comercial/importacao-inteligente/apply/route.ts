...
import { contractTypeMapFromForm, genericRepassMapFromForm, headerMapFromForm, parseSmartCommercialFile, SMART_IMPORT_ISSUES, validateSmartPolicyScope } from '@/lib/imports/smart-commercial-server'
...
export async function POST(req:Request){
 try{
  ...
  return Response.json({error:(m==='invalid_header_map'||m==='invalid_contract_type_map'||m==='invalid_generic_repass_map')?'Mapeamento manual inválido.':'Não foi possível concluir a importação.'},{status:(m==='invalid_header_map'||m==='invalid_contract_type_map')?400:500})
 }
}
