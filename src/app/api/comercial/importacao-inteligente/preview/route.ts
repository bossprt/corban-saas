...
export async function POST(req:Request){
 try{
  ...
  type Econ={table:string;contract:string;termMin:number;termMax:number;component:string;group:string;groupId:string;componentTypeId:string;mode:'share_of_received'|'direct'|'exclude';sharePct:string|null;receivedKind:'percentage'|'fixed_brl';gross:string;net:string;payout:string;retained:string|null;payoutKind:'percentage'|'fixed_brl'|null;compatible:boolean}
    ...
    for(const row of parsed.rows.slice(0,25)){
      for(const component of row.components){
        for(const item of items.filter(x=>x.component_type_id===component.component_type_id)){
          const key=[row.table_name,row.contract_type_id,row.term_min,row.term_max,component.component_type_id,item.group_id].join('|')
          ...
          economics.push({table:row.table_name,contract:row.contract_type_name,termMin:row.term_min,termMax:row.term_max,component:componentName.get(component.component_type_id)??'Componente',group:groupName.get(item.group_id)??'Grupo',groupId:item.group_id,componentTypeId:component.component_type_id,mode:item.mode,sharePct:item.share_pct==null?null:String(item.share_pct),receivedKind:component.value_kind,...calc})
        }
      }
    }
  }
  ...
  const repassComparisons=parsed.rows.flatMap(row=>row.source_repasses.filter(r=>r.source_slot&&r.rule_hint!=='unknown').map(r=>{
    const internal=economics.find(e=>e.table===row.table_name&&e.contract===row.contract_type_name&&e.termMin===row.term_min&&e.termMax===row.term_max&&e.groupId===r.group_id&&e.componentTypeId===r.component_type_id)
    if(!internal)return {table:row.table_name,contract:row.contract_type_name,termMin:row.term_min,termMax:row.term_max,groupId:r.group_id,componentTypeId:r.component_type_id,slot:r.source_slot,external:r.raw_value,externalKind:r.value_kind_hint,internal:null as string|null,status:'no_internal_rule' as const}
    if(r.rule_hint==='direct'){
      if(!r.value_kind_hint||internal.payoutKind!==r.value_kind_hint)return {table:row.table_name,contract:row.contract_type_name,termMin:row.term_min,termMax:row.term_max,groupId:r.group_id,componentTypeId:r.component_type_id,slot:r.source_slot,external:r.raw_value,externalKind:r.value_kind_hint,internal:internal.payout,status:'incompatible_unit' as const}
      return {table:row.table_name,contract:row.contract_type_name,termMin:row.term_min,termMax:row.term_max,groupId:r.group_id,componentTypeId:r.component_type_id,slot:r.source_slot,external:r.raw_value,externalKind:r.value_kind_hint,internal:internal.payout,status:decimalEqual(r.raw_value,internal.payout)?'match' as const:'different' as const}
    }
    if(internal.mode!=='share_of_received'||internal.sharePct==null)return {table:row.table_name,contract:row.contract_type_name,termMin:row.term_min,termMax:row.term_max,groupId:r.group_id,componentTypeId:r.component_type_id,slot:r.source_slot,external:r.raw_value,externalKind:'percentage' as const,internal:internal.sharePct,status:'incompatible_semantics' as const}
  ...
  return Response.json({
   ...
   repasses:parsed.rows.flatMap(r=>r.source_repasses.map(x=>({table:r.table_name,contract:r.contract_type_name,termMin:r.term_min,termMax:r.term_max,...x}))).slice(0,40),
   repassComparisons,
   sample:parsed.rows.slice(0,8).map(r=>({
    ...
    termMin:r.term_min,termMax:r.term_max,rate:r.rate,factor:r.factor_value,components:r.components.length
   })),
  })
 }catch(e){
