...
export default async function CasePage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{erro?:string;ok?:string}>}){
 ...
 return <section>
  ...
  {resolved&&c.resolved_at&&new Date(c.updated_at).getTime()>new Date(c.resolved_at).getTime()+1000&&<div role="status" className="mt-4 rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-sm text-amber-200">Os valores deste caso foram recalculados depois da resolução. A justificativa registrada é imutável e pode não cobrir os valores atuais: avalie com o ledger abaixo.</div>}
