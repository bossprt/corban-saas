...
export function SmartImportClient({providers}:{providers:Provider[]}){
 ...
 async function runPreview(map=repassMap()){
  if(!file)return
  setBusy(true);setMessage('')
  try{
   const fd=new FormData();fd.set('file',file);fd.set('repass_map',map)
 ...
 async function apply(){
  ...
  try{
   ...
   fd.set('file',file);fd.set('production_origin',origin);fd.set('provider_id',origin==='third_party'?provider:'');fd.set('repass_map',repassMap())
 ...
 const blocking=preview?.issues.filter(x=>x.code!=='generic_repass_requires_mapping')??[]
 return <div className="grid gap-4">
  <Card>
   ...
   <div className="px-5 pb-5 pt-2">
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2">
     <input type="file" accept=".csv,.xlsx,.xls,.pdf,text/csv" aria-label="Planilha" onChange={e=>{setFile(e.target.files?.[0]??null);setPreview(null);setSlots({});setMessage('')}} className="block flex-1 text-sm" />
     <button disabled={!file||busy} onClick={()=>{setSlots({});void runPreview('{}')}} className={primary}><Upload size={15} className="mr-1.5 inline" aria-hidden />{busy?'Lendo...':'Ler planilha'}</button>
    </div>
   </div>
  </Card>
  ...
  {preview&&!blocking.length&&<Card>
   ...
   <div className="grid gap-3 px-5 pb-5 pt-3">
    ...
    <div>{preview.issues.length>0&&<p className="mb-2 text-[13px] text-[#92400E]">Escolha o grupo de cada Repasse e clique em “Aplicar e ver a prévia de novo”.</p>}<button disabled={busy||preview.issues.length>0} onClick={apply} className={primary}>{busy?'Importando...':'Importar como rascunho'}</button></div>
   </div>
  </Card>}
