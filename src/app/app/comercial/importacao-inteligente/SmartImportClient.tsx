...
type Preview={
 ...
 economics:{table:string;contract:string;term:number;component:string;group:string;kind:'percentage'|'fixed_brl';gross:string;net:string;payout:string;retained:string|null;payoutKind:'percentage'|'fixed_brl'|null;compatible:boolean}[]
...
export function SmartImportClient({providers,policies}:{providers:Provider[];policies:Policy[]}){
 ...
 async function runPreview(){
  ...
  try{
   const fd=new FormData();fd.set('file',file);fd.set('policy_version_id',policy)
 ...
 return <div className="space-y-5">
  <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
   <h2 className="font-semibold">1. Envie a planilha e escolha a regra</h2>
   <p className="mt-1 text-xs text-slate-400">CSV ou XLSX, até 2 MB. A regra escolhida entra na prévia financeira; nada é gravado nesta etapa.</p>
   <div className="mt-3 grid gap-2 md:grid-cols-2"><select value={policy} onChange={e=>{setPolicy(e.target.value);setPreview(null)}} className={field+' md:col-span-2'}><option value="">Sem regra interna na prévia</option>{policies.map(p=><option key={p.versionId} value={p.versionId}>{p.name} · v{p.version} · imposto/desconto {p.discount}%</option>)}</select><input type="file" accept=".csv,.xlsx,text/csv" onChange={e=>{setFile(e.target.files?.[0]??null);setPreview(null);setMessage('')}} className="block text-sm"/><button disabled={!file||busy} onClick={runPreview} className={ghost}>{busy?'Analisando...':'Analisar sem gravar'}</button></div>
  </div>
  ...
  {preview&&<div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-5">
   ...
   {!!preview.economics?.length&&<div className="mt-5"><h3 className="text-sm font-semibold text-emerald-200">Prévia econômica — cenários por grupo</h3><p className="mt-1 text-xs text-slate-400">Cada grupo é um cenário alternativo de venda, não uma soma de repasses.</p><div className="mt-2 overflow-x-auto"><table className="min-w-[900px] w-full text-xs"><thead className="text-slate-500"><tr><th className="p-2 text-left">Tabela</th><th>Tipo/prazo</th><th>Componente</th><th>Grupo</th><th>Recebe bruto</th><th>Após imposto</th><th>Repasse</th><th>Empresa retém</th></tr></thead><tbody>{preview.economics.map((e,i)=><tr key={i} className="border-t border-slate-800"><td className="p-2">{e.table}</td><td>{e.contract} {e.term}x</td><td>{e.component}</td><td>{e.group}</td><td>{e.kind==='fixed_brl'?'R$ ':'% '}{e.gross}</td><td>{e.kind==='fixed_brl'?'R$ ':'% '}{e.net}</td><td>{e.payoutKind==='fixed_brl'?'R$ ':'% '}{e.payout}</td><td>{e.retained===null?'unidades diferentes':(e.kind==='fixed_brl'?'R$ ':'% ')+e.retained}</td></tr>)}</tbody></table></div></div>}
  </div>}
