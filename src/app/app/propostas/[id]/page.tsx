...
export default async function ProposalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <section>
    ...
    <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-900 p-5">
      ...
      {commercialRoute && ['admin','manager','supervisor'].includes(membership.role) && <form action={publishExpectedCommission}><input type="hidden" name="proposal_id" value={proposal.id}/><button className="rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-slate-950">Publicar comissão esperada</button></form>}</div>
