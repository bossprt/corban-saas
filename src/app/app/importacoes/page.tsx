...
export default async function ImportsPage() {
 ...
 <div className="space-y-2">{data.map(b=><div key={b.id} className="rounded-lg border border-slate-800 p-3 text-sm"><strong>{b.original_filename}</strong><span className="ml-3 text-slate-400">{b.status}</span><span className="ml-3 text-slate-500">{b.row_count??'—'} linhas</span></div>)}</div>}
 </div></section>
}
