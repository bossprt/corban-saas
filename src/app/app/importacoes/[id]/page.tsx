...
import { applyApprovedImportMatch, decideImportCandidate } from './actions'
import { selectImportAdapter } from '@/lib/imports/engine'
...
export default async function ImportBatchPage({params}:{params:Promise<{id:string}>}){
 ...
 const adapter=selectImportAdapter({filename:batch.original_filename,sourceKey:batch.parser_key?.startsWith('daycoval')?'daycoval':batch.parser_key?.startsWith('efetiva')?'efetiva_mais':batch.parser_key?.startsWith('bevicred')?'bevicred':undefined})
 const financialSemantic=adapter?.financialSemantic??'unclassified'
 return <section>
