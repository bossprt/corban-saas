...
export default async function ImportBatchPage({params}:{params:Promise<{id:string}>}){
 ...
 return <section>
  ...
  {conflicts.length>0&&<div className="mt-3 rounded-lg border border-amber-500/40 bg-amber-500/5 p-3 text-xs text-amber-200"><strong>{conflicts.length} achado(s) de conflito</strong> — nenhum publica fato financeiro; linhas afetadas exigem revisão humana.<ul className="mt-2 list-disc pl-5">{conflicts.slice(0,20).map((f,i)=><li key={i}>{f.kind}{f.identityKey?` · ${f.identityKey}`:''}{f.detail.dimension?` · ${String(f.detail.dimension)}: ${(f.detail.values as string[]).join(' ≠ ')}`:''} ({f.severity})</li>)}</ul></div>}
