...
import { ENVIRONMENT_LABEL, providerEnvironment, workerReadiness } from '@/lib/integrations/readiness'
import { atLeast } from '@/lib/rbac'
import { buildAttemptHistory, groupByRun } from '@/lib/integrations/attempt-history'
...
export default async function IntegrationsPage({ searchParams }: { searchParams: Promise<{ erro?: string; ok?: string }> }) {
  ...
  // Per-attempt history (immutable diagnostics). Best effort: a failure here never hides the runs themselves.
  const runIds = ((res.data ?? []) as { id: string }[]).map(r => r.id)
  const diagnostics = runIds.length && atLeast(membership.role, 'supervisor')
    ? await supabase.from('integration_run_artifacts').select('run_id,created_at,payload').eq('artifact_kind', 'diagnostic').in('run_id', runIds).order('created_at')
    : { data: null }
  const historyByRun = groupByRun((diagnostics.data ?? []) as { run_id: string; created_at: string; payload: unknown }[])
  const lastActivity = ((res.data ?? []) as { updated_at?: string | null; created_at: string }[]).map(r => r.updated_at ?? r.created_at).sort().pop() ?? null
  const readiness = atLeast(membership.role, 'supervisor') ? workerReadiness(process.env) : null
  ...
  return <section>
    ...
    {readiness && <div className="mt-6 rounded-xl border border-slate-800 bg-slate-900 p-4 text-sm">
      <div className="flex flex-wrap items-baseline justify-between gap-2"><strong>Prontidão do processamento</strong>
        <span className={readiness.secretConfigured && readiness.realProviderAvailable ? 'text-emerald-400' : 'text-amber-300'}>{readiness.secretConfigured && readiness.realProviderAvailable ? 'Pronto' : 'Não pronto para produção'}</span></div>
      <ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-slate-400">{readiness.notes.map(n => <li key={n}>{n}</li>)}</ul>
      <p className="mt-2 text-xs text-slate-500">Agendador: <strong className="text-slate-300">não verificado</strong>. {lastActivity ? `Última atividade de execução registrada em ${new Date(lastActivity).toLocaleString('pt-BR')} (evidência indireta, não prova que o agendador está ativo).` : 'Nenhuma execução registrada ainda.'}</p>
    </div>}

    <div className="mt-6 grid gap-3 md:grid-cols-2">
      {PROVIDER_REGISTRY.map(e => {
        const u = usability(e.manifest.adapterKey)
        const env = providerEnvironment(e.manifest.adapterKey)
        return <div key={e.manifest.adapterKey} className="rounded-xl border border-slate-800 bg-slate-900 p-4 text-sm">
          <div className="flex items-baseline justify-between gap-2"><strong>{e.manifest.adapterKey}</strong><span className={env === 'real' ? 'text-emerald-400' : env === 'local_test' ? 'text-sky-300' : 'text-amber-300'}>{ENVIRONMENT_LABEL[env]}</span></div>
          ...
          {env === 'local_test' && <p className="mt-2 text-xs text-sky-200">Provedor simulado para testes. Nenhum banco é consultado e o resultado nunca vira dado financeiro.</p>}
    ...
    <div className="mt-2 rounded-xl border border-slate-800 p-4 text-xs leading-5 text-slate-400">
      <strong className="text-slate-300">Como agir:</strong> <em>Nova tentativa</em> continua a mesma execução (respeita a espera e o limite de tentativas). <em>Nova execução</em> cria uma execução diferente ligada à anterior, com motivo, e só existe depois que a anterior terminou em falha definitiva ou foi cancelada. <em>Cancelar</em> só vale para execuções que ainda não estão em processamento; uma execução em andamento não pode ser interrompida.
    </div>
    ...
    {view.kind === 'ready' && <div className="mt-3 space-y-2">{(() => { const childOf = lineageOf(view.rows); return view.rows.map(r => {
      ...
      return <div key={r.id} className="rounded-xl border border-slate-800 bg-slate-900 p-4 text-sm">
        <div className="flex flex-wrap items-baseline justify-between gap-2"><strong>{adapter?.adapter_key ?? 'provedor'} · {r.capability}{adapter && providerEnvironment(adapter.adapter_key) === 'local_test' && <span className="ml-2 rounded border border-sky-500/40 px-1.5 py-0.5 text-[10px] font-normal text-sky-300">LOCAL / TESTE</span>}</strong><span className={phase === 'completed' ? 'text-emerald-400' : phase === 'needs_human' ? 'text-red-300' : 'text-slate-300'}>{PHASE_LABEL[phase]}</span></div>
        ...
        {(() => { const h = buildAttemptHistory(historyByRun.get(r.id)); return h.length > 0 && <details className="mt-2 text-xs text-slate-400"><summary className="cursor-pointer text-slate-300">Histórico de tentativas ({h.length})</summary>
          <ol className="mt-2 space-y-1">{h.map((a, i) => <li key={i} className="rounded border border-slate-800 px-2 py-1">
            <span className="text-slate-300">Tentativa {a.attempt}</span> · {a.label}{a.at ? ` · ${new Date(a.at).toLocaleString('pt-BR')}` : ''}
            {a.code && <span> · código {a.code}</span>}{a.message && <div className="mt-0.5 text-slate-500">{a.message}</div>}
          </li>)}</ol></details> })()}
