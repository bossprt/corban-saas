...
import { PHASE_LABEL, allowedActions, classifyRunsView, runPhase, type RunRow } from '@/lib/integrations/view-state'
...
const RICH=`${FULL},updated_at,parent_run_id,adapter:integration_adapters(adapter_key)`
  ...
  return <section>
    ...
    {view.kind === 'ready' && <div className="mt-3 space-y-2">{view.rows.map(r => {
      ...
      return <div key={r.id} className="rounded-xl border border-slate-800 bg-slate-900 p-4 text-sm">
        ...
        <div className="mt-1 text-xs text-slate-500">{r.correlation_id ? `correlação ${r.correlation_id}` : ''}{r.parent_run_id ? ` · nova execução da ${r.parent_run_id.slice(0, 8)}` : ''}</div>
