...
export function BankAccountRows({ initial = [] }: { initial?: AccountRow[] }) {
...
export function RegistrationRows({ agreements, initial = [] }: { agreements: { id: string; name: string }[]; initial?: RegistrationRow[] }) {
  const { rows, add, drop, toggle } = useRows(initial)
  return (
    <fieldset>
          ...
          return (
            <div key={r.key} className="grid gap-3 rounded-[12px] border border-line p-3 md:grid-cols-4" data-row="registration">
              ...
              <div className="flex items-center justify-between md:col-span-4">
                ...
                {!m && rows.length > 1 && <button type="button" onClick={() => drop(r.key)} className={removeBtn}><X size={13} aria-hidden />Remover</button>}
              </div>
              ...
              <label className={lbl}>Margem (R$)<input name="margin_amount" defaultValue={m?.margin_amount ? String(m.margin_amount).replace('.', ',') : ''} inputMode="decimal" placeholder="0,00" className="field mt-1.5" /></label>
