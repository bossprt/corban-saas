...
export default async function CustomerDetail({params}:{params:Promise<{id:string}>}){
 ...
 const [{data:customer},{data:proposals},{data:documents},{data:address}]=await Promise.all([
  ...
  supabase.from('client_addresses').select('zip,street,number,complement,district,city,state').eq('client_id',id).maybeSingle(), // null until the address migration is applied: the page keeps working
 ])
 ...
 return <section>
  ...
  <form action={saveCustomerAddress} className="mt-6 grid gap-3 rounded-xl border border-slate-800 bg-slate-900 p-5 md:grid-cols-5"><input type="hidden" name="client_id" value={customer.id} />
   ...
   <AddressFields initial={address?{zip:address.zip??'',street:address.street??'',number:address.number??'',complement:address.complement??'',district:address.district??'',city:address.city??'',state:address.state??''}:undefined} />
