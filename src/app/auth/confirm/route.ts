...
const TOKEN_SHAPE = /^[A-Za-z0-9_-]{8,256}$/ // reject junk before any network call; the value itself is never logged

export async function GET(request: NextRequest) {
  const tokenHash = request.nextUrl.searchParams.get('token_hash')
  const type = request.nextUrl.searchParams.get('type') ?? ''
  const fail = () => NextResponse.redirect(new URL('/login?erro=link', request.url))
  if (!tokenHash || !TOKEN_SHAPE.test(tokenHash) || !ALLOWED_TYPES.has(type)) return fail()
