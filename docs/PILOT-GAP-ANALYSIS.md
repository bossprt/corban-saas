...
| D-7 | `authenticated` holds INSERT/UPDATE/DELETE grants on `organizations` (no matching policies, so RLS refuses) | P2: revoke as defense in depth in a later migration |
Owner-only remainder: domain, Vercel project + env vars, Supabase Auth URLs + SMTP + password policy, reference-catalog data, Smart organization creation, commercial data (rates/coefficients/documents), commission decision, review of `20260929`.
