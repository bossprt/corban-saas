...
- **Adendo (25/09/2026):** the client is registered and edited in the same form (five blocks; several accounts and registrations; CPF never changes). The client page is read-only with an "Editar cadastro" button. Name, phone and e-mail change through `update_client_identity` (clientes.edit and sight of the client); the previous phone or e-mail stays in the contact history.

## ADR-0034 - Seller groups are payout rules per commission type (part A of the payout bridge)
- **Data:** 25/09/2026 - **Status:** aceita (decisões do dono); parte A implementada, partes B e C a seguir.
- **Decisão 1:** one grouping of sellers only. A seller group (table `commission_groups`: Corretor, Parceiro, Balcão, Call Center, Afiliado...) is the payout rule of the sellers in it, one to one: a seller picks one group and follows its rule. The ChatGPT-era second grouping (`seller_groups`) and the group question "% of received or % of production" are removed; groups must exist before the commission tables are imported.
- **Decisão 2:** a rule version (`commission_group_rules` + `commission_group_rule_items`, immutable, written only by `save_seller_group`) says, for each commission type the company receives (à vista, diferido, bônus 1-3, plástico, seguro), which table column the group reads (its own column, the company column or another group's column) and how many % of that value is paid out; plus supervisor and sales manager, each with a basis (spread = company receipt minus payout, total production, or the payout) and a %. Every company sets its own rules: nothing (like the owner's 65%/80%) is fixed in code.
...
## ADR-0036 - Commission tables by seller group (part B of the payout bridge)
- **Data:** 25/09/2026 - **Status:** aceita (decisões do dono); parte C a seguir.
- **Decisão 1:** every line of a commission table holds what the company receives per commission type and, per seller group and type, what that group gets (`commercial_condition_group_values`): a % of the operation or a fixed R$. Empty = the type does not apply to the group. Own-production groups have no values. Values are written only on drafts (`replace_condition_group_values`, `save_condition_values`), exact numeric; published vigências are immutable.
- **Decisão 2:** today's shares are converted once: value = company value x share / 100 (e.g. 40% of 5% = 2). The old shares stay until part C moves the calculation.
- **Decisão 3:** import follows the 2tech layout (company block + one block per group; number = %, "R$ 25,00" = fixed). "Repasse N" columns are mapped to a group (or "Não usar") by the person importing; an unknown group column refuses the whole file.
- **Decisão 4:** the tables screen is a search (filters, 10/25/50/100 per page) with row actions (vigências, commission, name); a table page shows its vigências and its lines one party at a time. A published vigência is changed through "Nova vigência a partir desta" (`clone_table_version`), edited line by line and published.
- **Próximo:** part C: the engine and the payout read the group values through the group rule; the bank-report import parameters (gear) come with the F5 reconciliation.

## ADR-0037 - Commission engine by table and seller group, tax per table line (part C1)
- **Data:** 26/09/2026 - **Status:** aceita (decisões do dono); C2 (alteração manual do repasse), C3 (crédito na conta com o contrato pago e o físico) e C4 (limpeza) a seguir.
- **Decisão 1:** the contract commission follows the table line and the seller's group rule (`calculate_contract_commission`): company receipt per type (% of gross or net, or fixed R$), seller = rule % of the column it reads, supervisor/manager by their basis, all to the cent. Proven with the owner's two real Hope contracts (1.235,00 and 352,43). The seller never pays tax; a payout above the receipt is refused.
- **Decisão 2:** tax is a column of each table line ("Imposto (%)", a fixed average of the Simples Nacional rate chosen by the owner); 0 = the company pays no tax on the line. It changes by exporting the table, editing and importing it back (a new vigência), or by hand on a draft. Whether a bank "pays tax" is read from its published tables (paid, exempt or mixed), never typed. The IR a bank withholds at source (Daycoval 0,5%) is typed in the bank registration.
- **Decisão 3:** margin = received − tax − IR withheld − seller − supervisor − manager, and may be negative. Each calculation keeps the rate, IR, group and rule it used; tables are versioned, so a later change never alters an earlier contract.
...
## ADR-0039 - Contract edits, seller payout change, notes and history (part C2)
- **Data:** 26/09/2026 - **Status:** aceita (decisões do dono); C3 (crédito na conta com o contrato pago e o físico) e C4 (limpeza) a seguir.
- **Decisão 1:** the contract file is its form (locked, "Editar contrato"): table/vigência, seller, gross, net, installment and term can change after the draft through `update_contract`; paid contracts only by the owner or a manager, with a reason. Every edit is recorded before -> after (`contract_events`) and the commission is recalculated at once; an edit the calculation cannot follow is refused as a whole. The ChatGPT-era guards that froze every field after the draft accept these fields only through that function.
- **Decisão 2:** the owner or a manager may change what the SELLER gets (not supervisor/manager), per commission type, as % of the type's base or fixed R$, with a reason (`set_payout_override`, rows never changed; a row without value brings the rule back). What is payable never exceeds what the company receives for the type, also after an edit. Finance sees rule, payable, margin and the gain; the seller sees only what they receive.
- **Decisão 3:** notes with author and date, never deleted, and one history (edits, notes, payout changes, stage moves). Payout changes are finance-only.
