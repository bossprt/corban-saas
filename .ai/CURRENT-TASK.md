...
- Grupos de vendedores = regra de repasse (ADR-0034, parte A): migration `20260925235900_seller_group_rules_v1` (NÃO aplicada em produção) cria regras versionadas por tipo de comissão (coluna de referência + % distribuído, supervisor e gerente com base spread/produção/repasse, produção própria), `save_seller_group`, e remove `seller_groups` (backup em `supabase/backups/20260925_seller_groups.sql`). Telas: Comercial > Grupos de vendedores (lista e grupo no padrão ficha = formulário), vendedor com um campo "Grupo". "Empresas de origem de terceiros" virou "Promotoras parceiras". `commission_component_types` entrou na lista de tabelas globais (antes a leitura falhava no modelo de planilha e na importação).
