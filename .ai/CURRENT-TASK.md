...
- Empresas de teste removidas de produção (decisão do dono 25/09/2026): migration `20260925150340_remove_test_tenants_v1` apagou "Tenant A/B - Teste RLS" (69 linhas de configuração padrão, vínculos e auditoria; sem dados de negócio) e o login de teste josicleuton@gmail.com. Backup em `supabase/backups/20260925_test_tenants.sql` (md5 conferido). Produção ficou só com a Smart Promotora. Contrato `remove-test-tenants-contract.sql` restaura o backup e prova a remoção.
- Grupos de vendedores = regra de repasse (ADR-0034, parte A): migration `20260925180643_seller_group_rules_v1` (aplicada em produção em 25/09/2026, md5 conferido) cria regras versionadas por tipo de comissão (coluna de referência + % distribuído, supervisor e gerente com base spread/produção/repasse, produção própria), `save_seller_group`, e remove `seller_groups` (backup em `supabase/backups/20260925_seller_groups.sql`). Telas: Comercial > Grupos de vendedores (lista e grupo no padrão ficha = formulário), vendedor com um campo "Grupo". "Empresas de origem de terceiros" virou "Promotoras parceiras". `commission_component_types` entrou na lista de tabelas globais (antes a leitura falhava no modelo de planilha e na importação).
- Menu "Comercial" virou "Cadastros" (decisão do dono 25/09/2026): sub-itens no menu lateral (Bancos, Convênios, Promotoras parceiras, Tipos de contrato, Tabelas, Grupos de vendedores, Vendedores, Fatores, Equipe) e na busca Ctrl K; central `/app/cadastros` no visual novo; a página antiga `/app/comercial` (onboarding e "Regra padrão de comissão") redireciona para lá; Equipe saiu de Configurações. Clientes continua item próprio.
- Ficha completa do vendedor (ADR-0035, etapa 1): migration `20260925221519_seller_registration_v1` (aplicada em produção em 25/09/2026, md5 conferido): código automático, dados básicos, contato, endereço residencial e comercial, contas PIX/TED com favorecido e histórico, contatos; `save_seller`. Tela: lista com "Novo vendedor" e ficha = formulário travado; Repasse mostra a conta principal ao pagar.
- Vendedores importados da 2tech (decisão do dono 25/09/2026): migration `20260925225917_seller_origin_v1` (aplicada em produção, md5 conferido) cria a origem do vendedor (`manual` ou `import:2tech`, com data). Carga única em produção: 76 vendedores (códigos 002–077, todos no grupo Corretor), 75 contas principais (72 PIX, 3 TED) com histórico "Importado da 2tech"; pulados a Luciane (já cadastrada) e o registro modelo "PRIMEIRO VENDEDOR" (conta fictícia); datas 01/01/1900 tratadas como vazias; celular antigo sem o 0; CNPJ = PJ. Os dados pessoais não foram para o repositório. A ficha e a lista mostram "Importado da 2tech".
- Parte B da ponte de repasse (ADR-0036): migration `20260926010227_commission_group_values_v1` (aplicada em produção 26/09/2026, md5 conferido; converteu 24.230 valores a partir dos percentuais atuais, pula 1.626 da Smart Promotora). Importação inteligente com mapeamento dos Repasses, tela de busca de tabelas (filtros, 10/25/50/100), detalhe com vigências e linhas por grupo, edição manual da comissão da linha em rascunho, nova vigência copiando a anterior, modelo de planilha novo. Leitura paginada (limite de 1.000 linhas da API).
- As 5 ações antigas de edição por bloco foram removidas (sem uso).
- Parte C1 da ponte de repasse (ADR-0037): migration `20260926035519_commission_engine_v2` (aplicada em produção 26/09/2026, md5 conferido). Cálculo novo pela linha da tabela e grupo do vendedor (Fabiola 1.235,00 e Elves 352,43 provados), Imposto (%) por linha (importação, modelo, exportação e formulário da linha), IR retido por banco, situação do imposto do banco lida das tabelas, base de cálculo obrigatória, tela Contratos com margem, tela Bancos no visual novo. Recebimento do banco deixa de creditar o vendedor (crédito vem na C3). Ficha do contrato como formulário passa para a C2.
- Hope com imposto (26/09/2026, pedido do dono): 13 tabelas próprias ganharam vigência nova com Imposto 6% e valores dos grupos = empresa × 0,94 × % do dono, truncado em 2 casas (219 valores, conferidos um a um). Daycoval só por promotora e Valor sem tabelas: sem mudança.
- Exportação por banco ou geral e "Nova pesquisa" (ADR-0038): migration `20260926061643_import_row_origin_v1` (aplicada em produção 26/09/2026, md5 conferido). Importação lê a origem de cada linha (coluna Promotora parceira) e envia em blocos de 500.
- Parte C2 (ADR-0039): migration `20260926161438_contract_changes_v1` (aplicada em produção 26/09/2026, md5 conferido). Ficha do contrato como formulário (editar tabela, vendedor, valores e prazo, recalculando; pago = só dono/gerente com motivo), alterar repasse do vendedor por tipo (% ou R$, motivo, nunca acima do recebido), observações e histórico único, trava quando o vendedor já recebeu, tela Contratos com selo Alterado, filtro e ganho.
- Contratos nascem calculados (ADR-0040): migration `20260926170000_auto_calculate_v1` (NÃO aplicada em produção). Cálculo automático ao criar (portal: ao aprovar); falha registrada no histórico com "Tentar de novo". Dois contratos reais da Hope cadastrados em produção (26/09/2026) com repasse alterado para 4% e 8%.
...
- Recebimento feito (ADR-0028): migration `20260924173938_commission_receipts_v1` (modelos de colunas por fonte e tipo, relatórios em conferência, conciliação ao centavo, lançamento imutável `commission_receipts`, aceite de divergência, `finance_alerts`). Contrato `tests/security/commission-receipts-contract.sql` 32/32. Telas: Financeiro (alertas + relatórios), Importar relatório (2 passos: ler colunas, importar), Conferência do relatório (vincular por ADE, ignorar com motivo, confirmar), Conciliação por contrato (aceitar divergência), recebido no cartão Comissão da proposta, 4 regras na Central de atenção/Hoje. Banco local recriado do zero: 17 contratos de segurança passando; unit 184/184; e2e 28/28 (fluxo completo de recebimento provado na tela). Aplicada em produção em 24/09/2026 (md5 conferido).
- Pendente F5: testar com relatório real de banco (à vista e diferido) quando o dono enviar; PR da F4 e da F5.
...
- Testado no banco local (curl: 401/201/200/422/403/429 corretos; log do servidor sem chave, telefone ou CPF). **Ambas aplicadas em produção** (md5 igual aos arquivos).
