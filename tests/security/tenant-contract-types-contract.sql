...
begin
 ...
 select count(*) into n from public.contract_types where organization_id is null and tech_key in ('novo','refinanciamento','compra_de_divida','portabilidade','refin_portabilidade');
 if n<>5 then raise exception 'global_contract_types_changed'; end if;
