-- Rollback-only behavioral contract for 20260919_reconciliation_cases_write_hardening_v1.sql (+ the token set by
-- refresh_financial_reconciliation in 20260919_financial_reversal_paths_v1.sql). Requires both applied (or executed
-- earlier in the same transaction). Ends with RAISE EXCEPTION so nothing persists; pass = message 'ALL PASS'.
do $test$
declare
...
begin
 ...
 perform pg_temp.run_as(u1,format($q$with u as (update public.financial_reconciliation_cases set status='resolved',resolution_note='checked with bank statement',resolved_by=%L where id=%L returning 1) select count(*)::text from u$q$,u3,c));
