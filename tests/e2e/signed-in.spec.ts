...
// ADR-0033: full client profile, a registration with its password (shown only on request) and a bank account.
test('client profile: personal data, registration with a vaulted password, bank account', async ({ page }, info) => {
  test.skip(info.project.name === 'mobile', 'one run is enough')
  ...
  await page.getByLabel('CPF').fill(cpf)
  await page.getByLabel('Telefone').fill('(68) 99911-2233')
  await page.getByLabel('Data de nascimento').fill('1980-05-17')
  await page.getByRole('button', { name: 'Cadastrar cliente' }).click()
  await expect(page.getByRole('heading', { name })).toBeVisible()
  await expect(page.getByText('Cadastro incompleto')).toBeVisible()
  await expect(page.getByText('17/05/1980')).toBeVisible()

  await page.locator('summary', { hasText: 'Editar dados pessoais' }).click()
  await page.getByLabel('Nome da mãe').fill('Maria da Silva')
  await page.getByLabel('Nome do pai').fill('José da Silva')
  await page.getByLabel('RG', { exact: true }).fill('123456')
  await page.getByLabel('Órgão expedidor').fill('SSP')
  await page.getByLabel('UF do RG').selectOption('AC')
  await page.getByLabel('Data de emissão').fill('2000-01-10')
  await page.getByLabel('Sexo').selectOption('F')
  await page.getByLabel('Estado civil').selectOption('married')
  await page.getByLabel('Naturalidade (cidade)').fill('Rio Branco')
  await page.getByLabel('UF de nascimento').selectOption('AC')
  await page.getByLabel('Mesmo número do telefone').check()
  await page.getByRole('button', { name: 'Salvar dados pessoais' }).click()
  await expect(page.getByText('Dados pessoais salvos.')).toBeVisible({ timeout: 30_000 })
  await expect(page.getByText('Maria da Silva')).toBeVisible()

  await page.locator('summary', { hasText: 'Adicionar matrícula' }).click()
  const reg = page.locator('details').filter({ hasText: 'Adicionar matrícula' })
  await reg.getByLabel('Convênio').selectOption({ label: 'INSS Teste' })
  await reg.getByLabel('Órgão').fill('Secretaria de Educação')
  ...
  await reg.getByRole('button', { name: 'Adicionar matrícula' }).click()
  await expect(page.getByText('Matrícula salva.')).toBeVisible({ timeout: 30_000 })
  ...
  await page.locator('summary', { hasText: 'Adicionar conta' }).click()
  const acc = page.locator('details').filter({ hasText: 'Adicionar conta' })
  ...
  await acc.getByRole('button', { name: 'Adicionar conta' }).click()
  await expect(page.getByText('Dados bancários atualizados.')).toBeVisible({ timeout: 30_000 })
  await expect(page.getByText('001 · Banco do Brasil')).toBeVisible()
