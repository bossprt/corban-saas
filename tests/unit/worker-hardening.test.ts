...
test('architecture: only server modules import admin/worker code, and the worker secret is read only by the dispatch route',()=>{
 ...
 // api/v1/leads only calls api_ingest_lead, which authenticates the company API key inside the database
 assert.deepEqual(importers,['src/app/api/admin/organizations/route.ts','src/app/api/admin/reference-catalog/route.ts','src/app/api/integrations/dispatch/route.ts','src/app/api/v1/leads/route.ts','src/app/app/integracoes/actions.ts','src/app/platform/actions.ts','src/app/platform/page.tsx','src/lib/appContext.ts','src/lib/integrations/worker.server.ts','src/lib/platform.server.ts','src/lib/team.server.ts'])
