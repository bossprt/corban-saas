-- CORBAN OS V2 — Auto-refresh reconciliation after approved import financial publication.
create or replace function public.publish_financial_fact_from_import_decision(p_decision_id uuid)
returns uuid language plpgsql security invoker set search_path=public as $$
declare v_org uuid;v_batch uuid;v_raw uuid;v_proposal uuid;v_semantic text;v_event_type text;v_amount numeric;v_component text;v_occurred timestamptz;v_ref text;v_candidate uuid;v_event uuid;
begin
 ...
 select c.proposal_id,n.amount,n.raw_row_id into v_proposal,v_amount,v_raw from public.import_match_candidates c join public.import_normalized_rows n on n.id=c.normalized_row_id and n.organization_id=c.organization_id where c.id=v_candidate and c.organization_id=v_org and c.match_strength='exact' and c.proposal_id is not null;
 ...
 select coalesce(nullif(n.normalized_payload->>'componentType',''),'upfront'),nullif(n.normalized_payload->>'occurredAt','')::timestamptz into v_component,v_occurred from public.import_normalized_rows n where n.raw_row_id=v_raw and n.organization_id=v_org order by n.created_at desc limit 1;
