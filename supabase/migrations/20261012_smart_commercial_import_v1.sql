-- CORBAN OS — Smart commercial import V1
-- PREPARED ONLY. Requires explicit Human Gate before LIVE apply.
-- Atomic import of banks/agreements/tables/draft versions/conditions/components/factors after application preview.
...
create table public.commercial_condition_component_policy(
  ...
  foreign key (organization_id,policy_version_id) references public.component_payout_policy_versions(organization_id,id) on delete restrict
);
create index commercial_condition_component_policy_version_idx on public.commercial_condition_component_policy(organization_id,policy_version_id);
create index commercial_condition_component_policy_attached_by_idx on public.commercial_condition_component_policy(attached_by) where attached_by is not null;

alter table public.commercial_condition_component_policy enable row level security;
revoke all on public.commercial_condition_component_policy from public,anon,authenticated;
grant select,insert,update on public.commercial_condition_component_policy to authenticated;
create policy commercial_condition_component_policy_select on public.commercial_condition_component_policy for select to authenticated
using (public.has_active_organization_role(organization_id,array['admin','manager','supervisor']));
create policy commercial_condition_component_policy_insert on public.commercial_condition_component_policy for insert to authenticated
with check (public.has_active_organization_role(organization_id,array['admin','manager']));
create policy commercial_condition_component_policy_update on public.commercial_condition_component_policy for update to authenticated
...
begin
  ...
  if v_status is distinct from 'draft' then raise exception 'published_version_component_policy_is_immutable'; end if;
  if current_user in ('authenticated','anon') and current_setting('corban.smart_import_rpc',true) is distinct from 'on' then
  ...
  ) then raise exception 'condition_component_policy_identity_is_immutable'; end if;
  if current_user in ('authenticated','anon') then new.attached_by:=auth.uid(); end if;
  return new;
end $$;
create trigger commercial_condition_component_policy_00_guard
before insert or update on public.commercial_condition_component_policy
for each row execute function public.guard_condition_component_policy();
...
begin
  ...
  for rowj in select * from jsonb_array_elements(p_rows) loop
    ...
    if v_bank_name is null or length(v_bank_name)>120
       ...
       or v_term not between 1 and 600
    ...
    where x.organization_id=p_organization and x.product_table_version_id=v_version.id and x.contract_type_id=v_contract.id and x.term=v_term
    limit 1;

    perform set_config('corban.condition_rpc','on',true);
    if v_condition.id is null then
      insert into public.commercial_conditions(organization_id,product_table_version_id,contract_type_id,term,coefficient,rate,created_by)
      values(p_organization,v_version.id,v_contract.id,v_term,v_coefficient,v_rate,auth.uid())
      returning * into v_condition;
    else
      update public.commercial_conditions set coefficient=v_coefficient,rate=v_rate,updated_at=now()
    ...
    if v_factor_value is not null then
      ...
      values(p_organization,v_factor_batch.id,v_term,v_term,v_factor_value,jsonb_build_object('smart_import',true))
