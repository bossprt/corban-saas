-- PREPARED, NOT APPLIED (Human Gate: replaces a live function and adds a unique index).
-- Found by the rollback-only E2E. Matching compares institutions case-insensitively
-- (lower(e.institution_key)=lower(r.bank_key)) but apply_approved_import_match looked up the existing identity with a
-- case-SENSITIVE equality. A row from a provider that spells the bank 'Daycoval' therefore did not find the stored
-- identity 'daycoval', INSERTED a second identity ('Daycoval') for the same proposal, and from then on the case-insensitive
-- matcher counted 2 identities -> 'ambiguous' -> every later statement/payment row for that proposal failed with
-- strong_match_required. Fix: compare with lower() in apply, store the institution key lower-cased, and forbid duplicates
-- that differ only by case with a unique index. Everything else in the function is unchanged (tenant still derived by the
-- decision; the existing `limit 1` membership lookup is documented debt B in docs/audits).
create unique index if not exists proposal_external_identities_org_inst_number_ci_uidx
 on public.proposal_external_identities(organization_id,lower(institution_key),external_proposal_number);
